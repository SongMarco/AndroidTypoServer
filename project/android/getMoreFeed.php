...
// 페이징을 위해 포스트로 보내온 마지막 피드로부터 10개의
// 뉴스피드를 불러오는 php
// 동작 방식은 getLikeList와 동일하되, 맨 처음의 쿼리문만 다름

$lastFeedId = $_POST['lastFeedNum'];


//$query = "select id, writer, title, text_content from dbMember2.table_newsfeed  where id < 20 order by id" ;

// 먼저 DB에서 "포스트로 보낸 마지막 피드 번호"보다 작은 최근 10개의 뉴스피드를 불러온다.

if ($lastFeedId != -1) {
    $query = "select * from dbMember2.table_newsfeed WHERE id < '$lastFeedId' order by id desc limit 10";
    $result = $db->query($query);

} else {
    $query = "SELECT * FROM dbMember2.table_newsfeed ORDER BY id DESC LIMIT 10";
    $result = $db->query($query);
}
